...
/**
 * Class encapsulating Bitcoin Candy startup and shutdown.
...
static void MigrateSettings() {
    ...
    // We only migrate settings if we have Core settings but no Bitcoin-Candy
    // settings
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    ...
    // Migrate settings from core's/our old GUI settings to Bitcoin Candy
    // only if core's exist but Bitcoin Candy's doesn't.
